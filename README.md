...
Normal scoring stops a verified building once accumulated visible length reaches its threshold and reports lower-bound coverage. Full diagnostic mode processes every antenna and reports complete coverage. The worker emits per-claim progress, supports cooperative cancellation between claims, and exports a versioned JSON report containing input SHA-256 hashes, pinned geometry versions, detailed counts, verified IDs, coverage results, and structured warnings.
